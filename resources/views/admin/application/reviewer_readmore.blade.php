    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            @if (!empty($reviewScore))
                ...
                @if (isEditor() || isSuperAdmin() || isReviewer())
                    <div class="mb-3">
                        <h6><strong>Message to Editor</strong></h6>
                        <p>{{ $reviewScore[16]['answer'] ?? 'N/A' }}</p>
                    </div>
                @endif

                @if (isSuperAdmin() || isReviewer())
                    <div class="mb-3">
                        <h6><strong>Message to Admin</strong></h6>
                        <p>{{ $reviewScore[19]['answer'] ?? 'N/A' }}</p>
                    </div>
                @endif

                @if (isSuperAdmin() || isReviewer())
                    <div class="mb-3">
                        <h6><strong>Final Remark</strong></h6>
                        <p>{{ $remark ?? 'N/A' }}</p>
                    </div>
                @endif
