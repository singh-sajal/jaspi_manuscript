@extends('author.app')
@section('title', 'ApplicManu Scriptation')
@section('page-title', 'New Manu Script')
...
@section('content')
    <div class="card">
        <form action="{{ route('author.application.store') }}" method="post" novalidate enctype="multipart/form-data">
            @csrf
...
@section('javascripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', toggleRequired);
    </script>
    <script>
        function toggleRequired() {
            const checkboxes = document.querySelectorAll('.article-checkbox');
